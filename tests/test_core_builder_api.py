...
def test_builder_create_update_delete_core_from_builder_shape():
    ...
    payload = {
        "node_id": node_id,
        "name": f"builder-builder-core-{suffix}",
        "enabled": True,
        "description": "created from builder builder shaped API test",
        "inbounds": [
            {
        ...
        "advanced_config": {"created_by": "builder_visual_builder"},
    }
    ...
    payload["name"] = f"builder-builder-core-updated-{suffix}"
    ...
    assert dry_run.json()["generated_config"]["version"] == "doctor-dev.v1"
