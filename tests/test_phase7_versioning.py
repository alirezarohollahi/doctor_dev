...
def _sample_core_payload(node_id: str, name: str = "phase7-versioning-core") -> dict:
    ...
    return {
        ...
        "name": name,
        "enabled": True,
        "description": "phase7 versioning test core",
        "inbounds": [
            {
                "id": inbound_id,
        ...
        "advanced_config": {"created_by": "phase7_test"},
    }


def test_phase7_dry_run_creates_version_and_diff_and_audit():
...
def test_phase7_rollback_restores_core_without_apply():
    client = TestClient(app)
    client.post("/api/dev/seed-local")
    node_id = client.get("/api/nodes").json()[0]["id"]
    create = client.post("/api/cores", json=_sample_core_payload(node_id, name="phase7-rollback-core"))
    assert create.status_code == 200, create.text
    core_id = create.json()["id"]

    v1 = client.post(f"/api/cores/{core_id}/dry-run").json()["version"]
    payload = _sample_core_payload(node_id, name="phase7-rollback-core")
    ...
    assert restored["description"] == "phase7 versioning test core"
