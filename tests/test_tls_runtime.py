...
def test_versioned_runtime_tls_listener_roundtrip():
    async def run() -> None:
        tls_port = free_port()
        echo_port = free_port()
        logs: list[tuple[str, str]] = []

        echo_server = await start_echo_server("127.0.0.1", echo_port, lambda level, message: logs.append((level, message)), label="versioned-test")
        manager = TunnelManager(lambda level, message: logs.append((level, message)))
        try:
            config = {
                "version": "doctor-dev.v1",
                "node_id": "node_test",
                "core_id": "core_tls",
                "core_name": "versioned-tls-runtime-test",
                "enabled": True,
                "advanced_config": {},
                "inbounds": [
                    {
            ...
            result = await asyncio.to_thread(tls_roundtrip, "127.0.0.1", tls_port, "hello-versioned-test", 5.0, False)
            assert result["ok"] is True
            assert result["mode"] == "tls"
            assert "doctor-dev-echo:versioned-test:hello-versioned-test" in result["response_text"]
