...
from .local_test import tcp_roundtrip, tls_roundtrip
...
async def upload_certificate(
    ...
    if len(fullchain_bytes) > 1024 * 1024 or len(privkey_bytes) > 1024 * 1024:
        raise HTTPException(status_code=400, detail="certificate files are too large for local panel upload")
...
@app.post("/api/test-lab/seed-local")
async def seed_local() -> dict[str, Any]:
    created = []
    specs = [
        (
            os.getenv("DOCTOR_DEV_NODE_A_NAME", "local-node-a"),
            os.getenv("DOCTOR_DEV_NODE_A_ADDRESS", "127.0.0.1"),
            int(os.getenv("DOCTOR_DEV_NODE_A_API_PORT", "9101")),
            os.getenv("DOCTOR_DEV_NODE_A_API_KEY", "local-dev-key-a"),
        ),
        (
            os.getenv("DOCTOR_DEV_NODE_B_NAME", "local-node-b"),
            os.getenv("DOCTOR_DEV_NODE_B_ADDRESS", "127.0.0.1"),
            int(os.getenv("DOCTOR_DEV_NODE_B_API_PORT", "9102")),
            os.getenv("DOCTOR_DEV_NODE_B_API_KEY", "local-dev-key-b"),
        ),
    ]
    existing_names = {node.name for node in store.list_nodes()}
    for name, address, api_port, key in specs:
        if name in existing_names:
            continue
        body = NodeCreate(name=name, address=address, node_port=62050, api_key=key, advanced=NodeAdvancedSettings(api_port=api_port))
        created.append(store.create_node(body).model_dump())
    return {"ok": True, "created": created, "total_nodes": len(store.list_nodes())}




def _get_or_create_tls_test_core() -> dict[str, Any]:
    nodes = store.list_nodes()
    if not nodes:
        raise HTTPException(status_code=400, detail="seed local nodes first")
    node = next((item for item in nodes if item.name == "local-node-a"), nodes[0])
    existing = [core for core in store.list_cores(node.id) if core.name == "tls-entry-core"]
    if existing:
        core = existing[0]
    else:
        route = RouteConfig(
            name="tls-entry-to-echo-target",
            balancer=BalancerType.round_robin,
            targets=[RouteTarget(type=TargetType.static, host="127.0.0.1", ports=[3000], priority=10, weight=1)],
        )
        inbound = InboundConfig(
            name="runtime-tls-public-entry",
            protocol="tcp",
            listeners=[InboundListener(listen_ip="127.0.0.1", listen_port=18443, public_host="127.0.0.1")],
            tls=CertificateRef(
                enabled=True,
                mode=CertificateMode.file_on_panel,
                domain="local.test",
                fullchain_path="certs/local.test/fullchain.pem",
                privkey_path="certs/local.test/privkey.pem",
            ),
            limits=InboundLimits(max_users=100, max_active_connections=50),
            route_id=route.id,
        )
        core = store.create_core(
            CoreCreate(
                node_id=node.id,
                name="tls-entry-core",
                enabled=True,
                description="TLS listener on 18443 terminates TLS and forwards TCP to a local echo target.",
                inbounds=[inbound],
                routes=[route],
                advanced_config={"created_by": "tls_runtime_test_core"},
            )
        )
    return {"node": node, "core": core, "test": {"host": "127.0.0.1", "port": 18443, "payload": "hello-doctor-dev-tls"}}


@app.post("/api/test-lab/create-tls-core")
async def create_tls_test_core() -> dict[str, Any]:
    item = _get_or_create_tls_test_core()
    return {
        "ok": True,
        "message": "TLS runtime core is ready",
        "node": item["node"].model_dump(),
        "core": item["core"].model_dump(),
        "test": item["test"],
    }


@app.post("/api/test-lab/apply-tls-core")
async def apply_tls_test_core() -> dict[str, Any]:
    item = _get_or_create_tls_test_core()
    result = await apply_core(item["core"].id)
    return {"ok": True, "message": "TLS runtime listener applied", "result": result, "test": item["test"]}

def _get_or_create_remote_route_cores() -> dict[str, CoreOut]:
    nodes = store.list_nodes()
    if len(nodes) < 2:
        raise HTTPException(status_code=400, detail="seed local nodes first; the remote route test needs local-node-a and local-node-b")
    node_a = next((node for node in nodes if node.name == "local-node-a"), nodes[0])
    node_b = next((node for node in nodes if node.name == "local-node-b"), nodes[1])

    b_existing = [core for core in store.list_cores(node_b.id) if core.name == "node-b-remote-exit-core"]
    if b_existing:
        b_core = b_existing[0]
        b_inbound = b_core.inbounds[0]
    else:
        b_route = RouteConfig(
            name="node-b-exit-to-echo-targets",
            balancer=BalancerType.weighted_round_robin,
            targets=[
                RouteTarget(type=TargetType.static, host="127.0.0.1", ports=[3100], weight=2),
                RouteTarget(type=TargetType.static, host="127.0.0.1", ports=[3101], weight=1),
            ],
        )
        b_inbound = InboundConfig(
            name="node-b-remote-group-inbound",
            protocol="tcp",
            listeners=[InboundListener(listen_ip="127.0.0.1", listen_port=19100, public_host="127.0.0.1")],
            limits=InboundLimits(max_users=100, max_active_connections=50),
            route_id=b_route.id,
        )
        b_core = store.create_core(
            CoreCreate(
                node_id=node_b.id,
                name="node-b-remote-exit-core",
                enabled=True,
                description="Remote target: node A can route to this inbound on node B, then node B routes to its echo targets.",
                inbounds=[b_inbound],
                routes=[b_route],
            )
        )

    a_existing = [core for core in store.list_cores(node_a.id) if core.name == "node-a-entry-core"]
    if a_existing:
        a_core = a_existing[0]
    else:
        a_route = RouteConfig(
            name="entry-to-node-b-remote-group",
            balancer=BalancerType.failover,
            targets=[
                RouteTarget(type=TargetType.remote_group, remote_node_id=node_b.id, remote_core_id=b_core.id, remote_inbound_id=b_inbound.id, priority=10, weight=3),
                RouteTarget(type=TargetType.static, host="127.0.0.1", ports=[3000], priority=50, weight=1),
            ],
        )
        a_inbound = InboundConfig(
            name="node-a-public-entry-inbound",
            protocol="tcp",
            listeners=[InboundListener(listen_ip="127.0.0.1", listen_port=18090), InboundListener(listen_ip="127.0.0.1", listen_port=18091)],
            limits=InboundLimits(max_users=20, max_active_connections=5),
            route_id=a_route.id,
        )
        a_core = store.create_core(
            CoreCreate(
                node_id=node_a.id,
                name="node-a-entry-core",
                enabled=True,
                description="Node A public inbound -> remote_group endpoint on node B -> node B echo target. Static local target is failover fallback.",
                inbounds=[a_inbound],
                routes=[a_route],
            )
        )
    return {"node_a": node_a, "node_b": node_b, "a_core": a_core, "b_core": b_core}


@app.post("/api/test-lab/create-local-chain-core")
async def create_local_chain_core() -> dict[str, Any]:
    # Local chain test core, useful for local-inbound testing.
    nodes = store.list_nodes()
    if not nodes:
        raise HTTPException(status_code=400, detail="seed local nodes first")
    node = nodes[0]
    existing = store.list_cores(node.id)
    if any(core.name == "local-chain-test-core" for core in existing):
        core = [core for core in existing if core.name == "local-chain-test-core"][0]
        return {"ok": True, "message": "local chain test core already exists", "core": core.model_dump()}

    internal_route = RouteConfig(
        name="internal-to-echo-targets",
        balancer=BalancerType.weighted_round_robin,
        targets=[
            RouteTarget(type=TargetType.static, host="127.0.0.1", ports=[3000], weight=2),
            RouteTarget(type=TargetType.static, host="127.0.0.1", ports=[3001], weight=1),
        ],
    )
    internal_inbound = InboundConfig(
        name="internal-hop-inbound",
        protocol="tcp",
        listeners=[InboundListener(listen_ip="127.0.0.1", listen_port=18100)],
        limits=InboundLimits(max_users=100, max_active_connections=50),
        route_id=internal_route.id,
    )

    public_route = RouteConfig(
        name="public-to-local-inbound",
        balancer=BalancerType.failover,
        targets=[RouteTarget(type=TargetType.local_inbound, local_inbound_id=internal_inbound.id, priority=10, weight=1)],
    )
    public_inbound = InboundConfig(
        name="public-entry-inbound",
        protocol="tcp",
        listeners=[InboundListener(listen_ip="127.0.0.1", listen_port=18080), InboundListener(listen_ip="127.0.0.1", listen_port=18081)],
        limits=InboundLimits(max_users=20, max_active_connections=5),
        route_id=public_route.id,
    )

    core = store.create_core(
        CoreCreate(
            node_id=node.id,
            name="local-chain-test-core",
            enabled=True,
            description="Local runtime test: public inbound -> local inbound -> static echo targets.",
            inbounds=[public_inbound, internal_inbound],
            routes=[public_route, internal_route],
        )
    )
    return {"ok": True, "core": core.model_dump()}


@app.post("/api/test-lab/create-remote-route")
async def create_remote_route() -> dict[str, Any]:
    items = _get_or_create_remote_route_cores()
    return {
        "ok": True,
        "message": "remote route test core is ready",
        "apply_order": [items["b_core"].id, items["a_core"].id],
        "node_a": items["node_a"].model_dump(),
        "node_b": items["node_b"].model_dump(),
        "entry_core": items["a_core"].model_dump(),
        "remote_exit_core": items["b_core"].model_dump(),
        "test": {"host": "127.0.0.1", "port": 18090, "payload": "hello-remote-route"},
    }


@app.post("/api/test-lab/apply-remote-route")
async def apply_remote_route() -> dict[str, Any]:
    items = _get_or_create_remote_route_cores()
    results = []
    # Apply remote exit first, then entry, so the remote listener is ready before node A points at it.
    for core in [items["b_core"], items["a_core"]]:
        result = await apply_core(core.id)
        results.append({"core_id": core.id, "core_name": core.name, "result": result})
    return {"ok": True, "message": "remote route applied: node-b exit first, then node-a entry", "results": results}
